...
<div class="max-w-3xl mx-auto h-screen">
    <div class="w-full  bg-neutral-800 mt-4 rounded ">
        <div class="flex items-start px-4 py-6 ">
            ...
            <div class="w-full">
                ...
                <div class="mt-4 flex items-center space-x-4 cursor-pointer">
                    <form class="flex mr-2 text-gray-500 text-sm myForm" method="POST"
                          action="/home/comment/create">
                        <input type="hidden" value="<?php echo $data['post_id'] ?>" name="id">
                        <button type="submit">
                        ...
                        <span> <?php echo $data['like_count'] ?></span>
                    </form>
        ...
        <form class="w-[90%] mx-auto mb-5 pb-2" method="POST" action="/comment">
            <div class="flex items-center border-b border py-2 px-2">
                <input class="appearance-none bg-transparent border-none w-full text-gray-300 mr-3 py-1 px-2 leading-tight focus:outline-none"
                       type="text" placeholder="comment..." aria-label="comment" name="comments">
                <input type="hidden" value="<?php echo $data['post_id'] ?>" name="id">
                <button
                        class="flex-shrink-0 text-sm border  text-white py-1 px-2 rounded"
                    ...
                    <div class="grow p-2 pb-8 flex justify-between">
                        ...
                        <?php if (isset($_SESSION['user']) && $_SESSION['user']['id'] === $post_comment['comment_user_id']): ?>
                            <div class="flex flex-col space-y-3">
                                <form class="size-8 rounded-full shadow bg-neutral-800 flex items-center justify-center"
                                      method="POST" action="/comment/delete">
                                    <input type="hidden" name="id"
                                           value="<?php echo $post_comment['comment_user_id'] ?>">
                                    <input type="hidden" name="cid" value="<?php echo $post_comment['comment_id'] ?>">
